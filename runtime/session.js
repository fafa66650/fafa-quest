import {TEAM_NAMES,ROLE_LIBRARY,DIFFICULTIES,SCENARIOS} from './data.js';
...
export function createSession(o){
  ...
  const scenarioId=cleanText(o.scenarioId,80),scenarioTitle=SCENARIOS.find(x=>x.id===scenarioId)?.title||scenarioId;
  const s={
    id:uid('session'),title:cleanText(o.title||scenarioTitle,80)||scenarioTitle,scenarioId,difficulty,
...
export function createImportedSession(payload){
  ...
  assignRoles(t);
  const scenarioId=cleanText(payload.scenarioId,80),scenarioTitle=SCENARIOS.find(x=>x.id===scenarioId)?.title||scenarioId;
  const s={id:uid('session'),title:cleanText(payload.title||scenarioTitle,80)||scenarioTitle,scenarioId,difficulty,durationMin,baseDurationMin:durationMin,playMode:'team_device',competition:'loisir',timerMode:'loisir',rotationMode:false,createdAt:Date.now(),startedAt:null,pausedAt:null,pausedTotalMs:0,teams:[t],worldCount:1,imported:true,version:8};
