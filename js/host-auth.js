...
export async function setHostPin(pin){
  ...
  storage.set(KEY,await digest(pin));
  sessionStorage.setItem(UNLOCK,'1');
  return true;
}
export async function verifyHostPin(pin){
  const ok=(await digest(pin))===storage.get(KEY);
  if(ok)sessionStorage.setItem(UNLOCK,'1');
  return ok;
}
export function lockHost(){sessionStorage.removeItem(UNLOCK)}
export function resetHostPin(){storage.remove(KEY);sessionStorage.removeItem(UNLOCK)}
