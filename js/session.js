...
export function createSession(o){
  ...
  const s={
    ...
    teams,worldCount:Math.max(1,o.worldCount||1),version:5
  };
...
export function createImportedSession(payload){
  const t={id:uid('team'),name:payload.team?.name||'Équipe',code:payload.team?.code||code(),members:payload.team?.members||[],roles:payload.team?.roles||{},teamKey:payload.team?.teamKey||'01',startWorldIndex:payload.team?.startWorldIndex||0};
  assignRoles(t);
  const s={id:uid('session'),title:payload.title||'Partie autonome',scenarioId:payload.scenarioId,difficulty:payload.difficulty||'aventure',durationMin:payload.durationMin||60,baseDurationMin:payload.durationMin||60,playMode:'team_device',competition:'loisir',timerMode:'loisir',rotationMode:false,createdAt:Date.now(),startedAt:null,pausedAt:null,pausedTotalMs:0,teams:[t],worldCount:1,imported:true,version:5};
